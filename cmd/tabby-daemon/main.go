...
func fetchSystemPaneSnapshotByWindow(sessionID string) map[string]windowSystemPaneSnapshot {
	...
	out, err := exec.Command("tmux", "list-panes", "-s", "-t", sessionID, "-F",
		"#{window_id}\x1f#{pane_id}\x1f#{pane_dead}\x1f#{pane_current_command}\x1f#{pane_start_command}").Output()
	if err != nil {
		return map[string]windowSystemPaneSnapshot{}
	}
...
func spawnRenderersForNewWindows(server *daemon.Server, sessionID string, windows []tmux.Window, coordinator *Coordinator) bool {
	...
	for _, win := range windows {
		...
		// Query pane state from a single session-wide snapshot (one tmux subprocess
		// per refresh cycle) instead of one list-panes call per window.
